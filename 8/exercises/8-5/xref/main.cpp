...
using std::cin;
using std::cout;
using std::endl; using std::numeric_limits; using std::streamsize; using std::sort;
using std::unique;
...
OutputIterator xref(istream&, OutputIterator, vector<string> (const string&) = split);
...
int main(int argc, char *argv[])
{
        ...
        xref(cin, out_it, find_urls);
        return 0;
}

template <class OutputIterator>
OutputIterator xref(istream& in, OutputIterator out_it, vector<string> find_words(const string&))
{
        string line;
        int line_number = 0;
        ...
        for (map<string, vector<int> >::const_iterator it=ret.begin(); it!=ret.end(); ++it) {
                // write the word
                *out_it = it->first;
                *out_it = "occurs on line(s):";

                // followed by one or more line numbers
                vector<int>::const_iterator line_it = it->second.begin();
                *out_it = std::to_string(*line_it);
